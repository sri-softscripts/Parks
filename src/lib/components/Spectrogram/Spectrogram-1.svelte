<script>
  ...
  const environments = [
    {
      name: 'Forest',
      icon: '/icons/icon-forest.svg',
      sound: '/audio/crickets.mp3',
      tips: [
        {
          image: '/images/flying-bird.png',
    ...
    {
      name: 'Desert',
      icon: '/icons/icon-desert.svg',
      sound: '/audio/Featured_Thunder.mp3',
      tips: [
        {
    ...
    {
      name: 'Lake',
      icon: '/icons/icon-lake.svg',
      sound: '/audio/WindRustlingLeaves.mp3',
      tips: []
    }
  ];
  ...
  function loadEnvironmentSound(environment) {
    if (!browser) return;
    
    loadingSound = true;
    loadingMessage = environment.name;
    selectedEnvironment = environment.name;
    currentTips = environment.tips;
    showTips = environment.tips.length > 0;
    ...
    AUDIO_ELEMENT = new Audio(environment.sound);
    AUDIO_ELEMENT.crossOrigin = "anonymous";
    AUDIO_ELEMENT.loop = true;

    AUDIO_ELEMENT.addEventListener('canplaythrough', function onCanPlay() {
      AUDIO_SOURCE = ACTX.createMediaElementSource(AUDIO_ELEMENT);
      ...
      AUDIO_ELEMENT.removeEventListener('canplaythrough', onCanPlay);
    });
  ...
  async function initVisualization() {
    ...
    renderer.setSize(spectrogramContainer.clientWidth, spectrogramContainer.clientHeight);
    spectrogramContainer.appendChild(renderer.domElement);
  ...
  function onWindowResize() {
    if (!browser) return;
    if (camera && renderer && spectrogramContainer) {
      camera.aspect = spectrogramContainer.clientWidth / spectrogramContainer.clientHeight;
  ...
  function update_geometry() {
    if (!browser) return;
    if (ACTX && ACTX.state === 'running' && ANALYSER && DATA && heights) {
      ANALYSER.getByteFrequencyData(DATA);
      let start_val = frequency_samples + 1;
      let end_val = n_vertices - start_val;

      if (heights.length >= n_vertices) {
        heights.copyWithin(0, start_val, n_vertices);
  ...
  function closeTip(index) {
    currentTips = currentTips.filter((_, i) => i !== index);
  ...
  <div class="page-content">
    {#if showTips}
      <div class="spectrogram-tips" id="spectrogram-section-1-tips">
        {#each currentTips as tip, index}
          <div class="spectrogram-tips__item">
            <a href="#" on:click|preventDefault={() => closeTip(index)} class="close-tip">Close</a>
            <div class="spectrogram-tips__item-img">
              <img loading="lazy" src={tip.image} alt="tip" />
            </div>
    ...
    <div class="section--1">
      <div class="container">
        <header class="page-header">
          ...
          <div class="sub-title">
            A spectrogram is a graphical representation of acoustic
            measurements. Acoustic spectrograms have three axes:
            <strong>time, frequency, and intensity</strong>.
          </div>
        </header>
      </div>
    </div>
    ...
    <div class="spectrogram-section" id="spectrogram-section-1">
      <div class="container">
        <div class="spectrogram-header">
          ...
          <div class="sub-title">
            Composed of the natural sound conditions in a park that
            exist in the absence of any human-made noise (e.g.
            mechanical, electrical, and other non-natural sounds).
          </div>
        </div>
            ...
            <div class="spectrogram-buttons">
              {#each environments as environment}
                <div
                  ...
                  class:active={selectedEnvironment === environment.name}
                  on:click={() => loadEnvironmentSound(environment)}
                  on:keydown={(e) => e.key === 'Enter' && loadEnvironmentSound(environment)}
                  role="button"
                  tabindex="0"
                  data-name={environment.name}
                  data-src={environment.sound}
                  id={environment.name === 'Forest' ? 'trigger-forest' : ''}
                >
                  <div class="spectrogram-button-icon">
                    <img loading="lazy" src={environment.icon} alt={environment.name} />
                  </div>
                  <div class="spectrogram-button-text">{environment.name}</div>
                </div>
              {/each}
            </div>
          </div>
        ...
        <div class="spectrogram-set">
          <!-- Changed from canvas to div for Three.js container -->
          <div bind:this={spectrogramContainer} id="spectrogramcanvas" style="width: 100%; height: 500px;"></div>
          <canvas id="legend"></canvas>
          
          {#if loadingSound}
            <div id="loadingSound" class="music-box__modal">
              <div class="music-box__modal__message">
                <div id="loadingMessage" class="music-box__modal__message__text">
                  {loadingMessage}
                </div>
                <svg
                  class="spinner"
