<script lang="ts">
	...
	} from '../data/functional-effects-data';

	let mode: 'animals' | 'humans' = 'animals';
	let visiblePanelId: string | null = null;
	let showInfoPanel = false;

	function handleModeChange(event: Event) {
		const target = event.target as HTMLInputElement;
		mode = target.value as 'animals' | 'humans';
		visiblePanelId = null;
		showInfoPanel = false;
	}

	function handleHotspotClick(panelId: string) {
		console.log('handleHotspotClick called with panelId:', panelId);
		visiblePanelId = panelId;
		showInfoPanel = true;
		console.log('Updated visiblePanelId to:', visiblePanelId);
	}
	...
	function handleNavigate(target: string) {
		visiblePanelId = target;
		showInfoPanel = true;
	}

const currentPanels = $derived(
	mode === 'animals' ? animalPanels : humanPanels
);

const currentPanorama = $derived(
	mode === 'animals' ? animalsPanorama : humansPanorama
);

const visiblePanel = $derived(
	currentPanels.find((p) => p.id === visiblePanelId)
);
	
	$effect(() => {
		if (visiblePanelId) {
			console.log('Visible panel ID changed to:', visiblePanelId);
			console.log('Current panels:', currentPanels.map(p => p.id));
			console.log('Found panel:', visiblePanel);
		}
	});
</script>

<div class="page-inner">
	<div class="page-content" class:pano class:show-info-panel>
		<div class="overlay-360" style="display: none;">
		...
		<div
			...
			class:active={mode === 'animals'}
			style="background-color: rgb(0, 0, 0);"
		>
			<PanoramaViewer
				panoramaImage={animalsPanorama.image}
				hotspots={animalsPanorama.hotspots}
		...
		<div
			...
			class="pano-container"
			class:active={mode === 'humans'}
			style="background-color: rgb(0, 0, 0);"
		>
			<PanoramaViewer
				panoramaImage={humansPanorama.image}
				hotspots={humansPanorama.hotspots}
				onHotspotClick={handleHotspotClick}
				active={mode === 'humans'}
			/>
		</div>
		...
		{#each currentPanels as panel (panel.id)}
			<InfoPanel
				{panel}
				visible={visiblePanelId === panel.id}
	...
	<footer class="page-footer">
		<div class="container">
			...
			>
				<img src="/src/lib/assets/icons/arrow-down.svg" width="22" height="11" alt="" />
			</button>
		</div>
	</footer>
</div>
...
<style>
	.page-content.show-info-panel {
		/* Add any styles needed when info panel is shown */
	}
</style>
