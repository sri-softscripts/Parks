<script lang="ts">
  ...
  import ThreeSceneManager from './ThreeSceneManager.svelte';
  import HotspotInfoPanel from './HotspotInfoPanel.svelte';
  import OptimizedImage from '../OptimizedImage.svelte';
  import { functionalEffectsData, type Category } from '$lib/data/functional-effects-data';
  
  export let selectedCategory: Category = 'animals';
  ...
  let bgTextures: Record<Category, THREE.Texture> | null = null;
  let texturesLoaded = false;
  let currentTexture: THREE.Texture | null = null;
  
  $: hotspots = functionalEffectsData[selectedCategory];
  
  onMount(() => {
    // Initialize texture loader
    textureLoader = new THREE.TextureLoader();
    
    // Create loading manager to track texture loading
    ...
    // Create new texture loader with manager
    ...
    try {
      ...
      // Set texture parameters and update current texture
      if (bgTextures) {
        ...
        currentTexture = bgTextures[selectedCategory];
      }
    } catch (error) {
  ...
  // Update current texture when category changes
  $: if (bgTextures && selectedCategory) {
    currentTexture = bgTextures[selectedCategory];
    console.log('Texture changed to:', selectedCategory, currentTexture);
  }
...
{:else}
  ...
  <div bind:this={container} class="viewer">
    {#if currentTexture && currentTexture.image && currentTexture.image.complete}
      <ThreeSceneManager
        {container}
        {hotspots}
...
{#if activeIndex > -1 && hotspots[activeIndex]}
  <HotspotInfoPanel
