<section id="physics-of-sound" class="">
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 const soundChart = new Chart(ctx, {
   ...
   data: {
     labels: [
       '', // empty label to create left space before the first real point
     ...
     datasets: [{
       data: [
         null, // no point at the empty label
         0,    // first real point starts at 0 dB (ear icon)
         15,
         25,
         40,
         60,
       ...
       backgroundColor: 'rgba(139, 69, 19, 0.95)', // exact dark brown fill
       borderColor: '#FFA500',                     // exact orange line
       borderWidth: 4,
       pointRadius: 7,
       pointBackgroundColor: '#FFA500',            // solid orange dots - no white border
       fill: true,
       tension: 0.1                                // almost straight line with very slight smoothing
     }]
   },
   options: {
     responsive: false,
 ...
 audioButtons.forEach(button => {
   ...
   button.addEventListener('click', function() {
     if (audio.paused) {
       ...
       document.querySelectorAll('.play').forEach(p => p.style.display = 'inline');
       document.querySelectorAll('.pause').forEach(p => p.style.display = 'none');
